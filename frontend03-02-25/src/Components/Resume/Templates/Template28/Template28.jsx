...
// import styles from "./Template28.module.css";
          ...
          <section className={styles.section}>
            ...
            {info.education.map((edu, index) => (
              <div key={index} className={styles.entry}>
                <strong>{edu.title}</strong>
                <p>
                  {edu.college} | {edu.startDate ? new Date(edu.startDate).getFullYear() : "Start"} -{" "}
                  {edu.endDate ? new Date(edu.endDate).getFullYear() : "Present"}
                </p>
        ...
        {info.workExp.length > 0 && (
          <section className={styles.section}>
            ...
            {info.workExp.map((work, index) => (
              <div key={index} className={styles.entry}>
                <strong>{work.title}</strong> - {work.companyName} (
                {work.startDate ? new Date(work.startDate).getFullYear() : "Start"} -{" "}
                {work.endDate ? new Date(work.endDate).getFullYear() : "Present"})
